    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                coco_eval.summarize()
                if self.classwise:
                    # Compute per-category AP and Recall
                    precisions = coco_eval.eval['precision']
                    recalls = coco_eval.eval['recall']
                    assert len(self.cat_ids) == precisions.shape[2]

                    # Extract IoU=0.5 results
                    iou_indexes = np.where(np.array(coco_eval.params.iouThrs) == 0.5)
                    if iou_indexes[0].size == 0:
                        # Handle the case where iou=0.5 is not found
                        print("IoU threshold 0.5 not found in evaluation parameters.")
                        # 你可以根据需要进行处理，如跳过或使用默认值
                    else:
                        iou_index = iou_indexes[0]
                    # maxDet indices for Recall@1, @10, @100
                    max_det_indices = [coco_eval.params.maxDets.index(n) 
                                      for n in [1, 10, 100]]

                    results_per_category = []
                    headers = ['category', 'mAP_50', 'Recall@1', 'Recall@10', 'Recall@100']
                    for idx, cat_id in enumerate(self.cat_ids):
                        ...
                        mAP_50 = np.mean(precision) if precision.size else np.nan
                        t.append(round(mAP_50, 3))

                        # Compute Recall@1, @10, @100
                        for md_idx in max_det_indices:
                            recall = recalls[0, idx, 0, md_idx]
                            recall = recall[recall > -1]
                            score = np.mean(recall) if recall.size else np.nan
                            t.append(round(score, 3))

                        results_per_category.append(t)
