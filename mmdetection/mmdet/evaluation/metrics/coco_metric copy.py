        ...
        for metric in self.metrics:
            if metric == 'bbox':
                ...
                for idx, cat_id in enumerate(self.cat_ids):
                    ...
                    # 计算AP50：使用最大的maxDets（100）
                    precision = precisions[0, :, idx, 0, -1]
                    ap_50 = np.mean(precision) if precision.size else float('nan')
                    eval_results[f'{nm["name"]}_AP50'] = round(ap_50, 3)
                    
                    # 计算每个proposal_num的Recall
                    for proposal_num in self.proposal_nums:
                        recall_idx = self.proposal_nums.index(proposal_num)
                        recall = recalls[0, idx, 0, recall_idx]
                        avg_recall = np.mean(recall) if recall.size else float('nan')
                        eval_results[f'Recall@{proposal_num}_category_{nm["name"]}'] = round(avg_recall, 3)
                        
                        # 打印验证
                        print(f"Category {nm['name']} Recall@{proposal_num}: {avg_recall:.4f}")
                        
                    results_per_category.append((nm["name"], ap_50))
                
                # 打印总结表格
                headers = ['Category', 'AP50', 'Recall@1', 'Recall@10', 'Recall@100']
                table_data = [headers]
                for cat_name, ap50 in results_per_category:
                    row = [cat_name, round(ap50,3), eval_results.get(f'Recall@1_category_{cat_name}', 'N/A'),
                        eval_results.get(f'Recall@10_category_{cat_name}', 'N/A'),
                        eval_results.get(f'Recall@100_category_{cat_name}', 'N/A')]
