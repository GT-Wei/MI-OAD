...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        # Add the first label to top-left corner
        # if len(unique_names) > 0:
        #     unique_names = unique_names[0]

        #     top_left_pos = np.array([10, 10])  # Position at top-left corner
        #     self.draw_texts(
        #         f'Prompt: {unique_names}',
        #         top_left_pos,
        ...
        # Add the first label to top-left corner
        if len(unique_names) > 0:
            unique_name = f'Prompt: {unique_names}'
            top_left_pos = np.array([10, 10])  # Position at top-left corner

            # 获取图像尺寸
            image_height, image_width = self._image.shape[:2]

            # 假设font_size为12，需要根据实际设置调整
            font_size = 18
            # 使用OpenCV来获取字体宽度
            font = cv2.FONT_HERSHEY_SIMPLEX
            # 计算单行最大字符数
            _, max_char_width = cv2.getTextSize(' ', font, font_size/4, 1)[0]
            max_chars_per_line = int(55)

            # 分割文字内容
            words = unique_name.split()
            lines = []
            current_line = []
            for word in words:
                # 计算当前行加上新词的长度
                current_line_length = sum([len(w) for w in current_line])
                new_length = current_line_length + len(word) + 1  # 加1代表空格
            ...
            for line in lines:
                # 调整位置偏移量
                current_pos = top_left_pos.copy()
                current_pos[1] += y_offset
                y_offset += font_size + 10  # 2为行间距
    ...
    def add_datasample(
        ...
        filtered_names = [name for name in data_sample.pred_instances.label_names if name != '#']
        unique_names = []
        seen = set()
        for name in filtered_names:
            if name not in seen:
                ...
                seen.add(name)
        # data_sample.gt_instances.label_names = unique_names * len(data_sample.gt_instances.labels)
